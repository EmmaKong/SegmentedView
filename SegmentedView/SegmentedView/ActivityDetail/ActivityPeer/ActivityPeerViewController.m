...
@interface ActivityPeerViewController ()<PullingRefreshTableViewDelegate>
...
@implementation ActivityPeerViewController
...
{
    // json数据解析
    NSString *activitypeerPath = [[NSBundle mainBundle] pathForResource:@"activitypeer" ofType:@"json"];
    NSData *data = [NSData dataWithContentsOfFile:activitypeerPath];
    NSArray *array = [NSJSONSerialization JSONObjectWithData:data options:NSJSONReadingAllowFragments error:nil];
    _activitypeersArray = [[NSMutableArray alloc] init];
    for (int i = 0; i < array.count; i++) {
        Member *activitypeer = [[Member alloc] initWithPropertiesDictionary:array[i]];
        
        [_activitypeersArray  addObject:activitypeer];
    }
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    return _activitypeersArray .count;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    Member *activitypeer = _activitypeersArray[indexPath.row];
    
    NSString *nametext = [NSString stringWithFormat:@"%@",activitypeer.name];
    NSString *somethingtext = [NSString stringWithFormat:@"%@",activitypeer.something];
